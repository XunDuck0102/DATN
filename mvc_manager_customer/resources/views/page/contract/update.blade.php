...
@section('content')
    <main class="content" style="margin-top: -65px; margin-left: -40px">
        <div class="container-fluid p-0">
            <div class="row mb-4">
                <div class="col-12">
                    <h1 class="h3">Chỉnh sửa hợp đồng</h1>
                </div>
            </div>
                            ...
                            <div class="card-body">
                                    ...
                                    <select name="staff_id" class="form-select" required>
                                        ...
                                        @foreach($staffs as $user)
                                            <option value="{{ $user->id }}"
                                                {{ $user->id == $contract->staff_id ? 'selected' : '' }}>
                                                {{ $user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea name="content" rows="4" class="form-control"
                                              placeholder="Nhập nội dung chi tiết hợp đồng...">{{ old('content', $contract->content) }}</textarea>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="date" name="signed_date" class="form-control"
                                           value="{{ old('signed_date', $contract->signed_date) }}">
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    @if(!empty($contract->file))
                                        <div class="mt-2">
                                            <a href="{{ $contract->file }}" target="_blank"
                                               class="btn btn-sm btn-outline-secondary">Xem file hiện tại</a>
                                        </div>
                                    @endif
                                </div>
